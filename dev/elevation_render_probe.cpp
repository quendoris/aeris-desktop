...
[[nodiscard]] bool prove_surface(
    ...
    const QImage with_image = render_model(
        application,
        model,
        project_uuid,
        revision,
    ...
    const QImage without_image = render_model(
        application,
        without_elevation,
        project_uuid,
        revision,
    ...
    if (with_image.isNull() || without_image.isNull()) {
        std::cerr << "terrain pixel proof produced a null image\n";
        return false;
    }

    const std::size_t changed = changed_pixels(with_image, without_image);
    if (changed < kMinimumChangedPixels) {
        std::cerr
            << aeris::view::surface_mode_name(mode)
            << " elevation visibility changed only " << changed
            << " pixels; terrain renderer appears disconnected from the map stack\n";
        return false;
    }

    std::cout
        << aeris::view::surface_mode_name(mode)
        << " terrain pixels: PASS changed=" << changed << '\n';
    return true;
}
...
int main(int argc, char** argv) {
    ...
    std::cout
        ...
        << "terrain changes durable Globe and Sinu-Mollweide pixels\n";
    return EXIT_SUCCESS;
}
